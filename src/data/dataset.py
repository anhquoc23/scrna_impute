...
def load_labels_csv(path, n_cells=None):
    """Nạp nhãn cụm từ file hai cột `cell, cluster` của Khối 1.

    Trả về mảng (C,) int64. Nếu path là None thì trả về toàn số 0.
    """
    if path is None:
        return np.zeros(int(n_cells or 0), dtype=np.int64)
...
def load_training_data(x_csv, m_csv, label_csv=None):
    ...
    label_csv : nhãn cụm (CSV) hoặc None.
    ...
    if bool(config.VERBOSE):
        rate = float(M.mean()) * 100.0
        print(f"[data] {data['n_cells']} tế bào x {data['n_genes']} gene | "
              f"dropout {rate:.2f}% | {data['n_clusters']} cụm")
    return data
...
def simulate_mask(M, epoch):
    ...
    u = torch.rand(M.shape, generator=g)
...
def iter_batches(data, idx, epoch=0, shuffle=None, m_sim=None):
    ...
    if shuffle:
        ...
        order = idx[torch.randperm(n, generator=g)]
...
def build_data(X, M, labels, cell_names, gene_names, verbose=None):
