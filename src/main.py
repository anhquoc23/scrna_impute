...
def write_csv(arr_cg, cell_names, gene_names, path):
    ...
    if hasattr(arr_cg, "detach"):
        arr_cg = arr_cg.detach().cpu().numpy()   # .cpu() bắt buộc khi chạy GPU
...
def build_models(data):
    """Tạo Generator và Discriminator, đưa lên cùng thiết bị với dữ liệu."""
    dev = data["device"]
    gen = build_generator(data["n_genes"]).to(dev)
    disc = build_discriminator(data["n_genes"], n_clusters=max(data["n_clusters"], 1)).to(dev)
    return gen, disc
...
def print_model_info(gen, disc):
    ...
    print(f"Thiết bị: {next(gen.parameters()).device}")
...
def save_checkpoint(gen, disc, path):
    ...
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    # đưa về CPU trước khi lưu, để máy không có GPU vẫn nạp lại được
    torch.save({"generator": {k: v.cpu() for k, v in gen.state_dict().items()},
                "discriminator": {k: v.cpu() for k, v in disc.state_dict().items()},
...
def parse_args(argv=None):
    ...
    g4.add_argument("--device", choices=("auto", "cuda", "cpu"), default=None,
                    help="auto = có GPU thì dùng (mặc định); cuda = bắt buộc GPU; "
                         "cpu = ép chạy CPU")
